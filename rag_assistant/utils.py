...
def get_or_create_vectorstore(pdf_dir, persist_dir, force_rebuild=False):
    ...
    # Попытка подключения к существующей базе
    if not force_rebuild:
        try:
            ...
            print(f"[INFO] Найдено {count} векторных документов в базе")
            if count > 0:
                return vectordb
        except Exception as e:
            print(f"[WARN] Ошибка загрузки существующей базы: {e}")

    # Пересоздание базы
    for attempt in range(3):
        try:
            if force_rebuild:
                print(f"[INFO] Пересоздание векторной базы. Удаляем {persist_dir}")
                if os.path.exists(persist_dir):
                    shutil.rmtree(persist_dir, ignore_errors=True)

            os.makedirs(persist_dir, exist_ok=True)

            print("[INFO] Парсим документы...")
            docs = parse_pdfs(pdf_dir)
            if not docs:
                raise ValueError("❌ Не удалось загрузить ни одного документа. Прерывание.")

            print("[INFO] Разбиваем на чанки...")
            splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=100)
            chunks = splitter.split_documents(docs)
            print(f"[INFO] Итоговое число чанков: {len(chunks)}")
            ...
            print(f"[RETRY] Ошибка доступа (PermissionError): {e}")
            if attempt == 2:
                raise
            time.sleep(1)

        except Exception as e:
            print(f"[ERROR] Ошибка создания базы: {e}")
            if attempt == 2:
                raise
            time.sleep(1)